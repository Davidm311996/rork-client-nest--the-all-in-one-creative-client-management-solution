...
export default function ClientVerifyScreen() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (code === '123456') {
        // Create client session and navigate to client dashboard
        Alert.alert(
          'Welcome!',
          'Your email has been verified. Welcome to Client Nest!',
          [
            {
              text: 'Continue',
              onPress: () => router.replace({ pathname: '/client-dashboard' })
            }
          ]
        );
      } else {
  ...
  const handleResendCode = async () => {
    ...
    try {
      // Mock resend - in real app this would resend code
      setResendTimer(30);
      setCanResend(false);
